trigger CustomerProjectTeamMemberTrigger on Project_Team_Member__c (before insert, before update, after insert, after update) {
    ...
    if(Trigger.isBefore){
        ...
        Boolean isSandbox = [select IsSandbox from Organization where Id =:UserInfo.getOrganizationId()].IsSandbox;
        for(Project_Team_Member__c ptm : Trigger.New){
            ptm.User__c = conUserMap.get(ptm.Contact__c) != null ? conUserMap.get(ptm.Contact__c) : null;
            
            String imgUrl = '/profilephoto/005/T';
            if(conUserImageMap.get(ptm.Contact__c) != null){
                imgUrl = conUserImageMap.get(ptm.Contact__c);
            }
            imgUrl = imgUrl.replace('https://', '');
            imgUrl = imgUrl.replace('/success', '');
            List<String> urlArray = imgUrl.split('/');
            urlArray[0] = isSandbox ? '/success' : '';
